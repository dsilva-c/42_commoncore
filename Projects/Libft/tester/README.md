...
- .../Libft/        ← Libft project root (contains `libft.h` and `libft.a` after building)  
- .../Libft/tester/ ← this tester folder
...
2) Ensure `LIBFT_DIR` points to the `Libft` directory
...
Minimal checklist
- Run `make` inside `Libft` to produce `libft.a`.
- Ensure `LIBFT_DIR` will point to the `Libft` path when you build the tester.
