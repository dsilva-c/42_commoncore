...
Generates realistic test data for space station monitoring, alien contacts,
and crew missions.
"""
...
class AlienContactGenerator:
    ...
    def generate_contact_data(self, count: int = 8) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            contact_id = (
                f"AC_{self.config.base_date.year}_"
                f"{str(i + 1).zfill(3)}"
            )
...
class CrewMissionGenerator:
    ...
    def generate_crew_member(self, member_id: str) -> Dict[str, Any]:
        ...
        years_experience = min(
            base_experience + rank_bonus[rank] + random.randint(-2, 3),
            30,
        )
    ...
    def generate_mission_data(self, count: int = 3) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            mission_type = (
                "Colony" if random.random() < 0.5 else "Research"
            )
            mission_name = f"{destination} {mission_type} Mission"
...
def main() -> None:
    ...
    for station in stations:
        ...
        print(
            f"  {station['station_id']}: {station['name']} - {status}"
        )
    ...
    for contact in contacts:
        verified = (
            "✅ Verified" if contact["is_verified"] else "❓ Unverified"
        )
        print(
            f"  {contact['contact_id']}: {contact['contact_type']} "
            f"at {contact['location']} - {verified}"
        )
